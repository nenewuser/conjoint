...
if ("Supporta" %in% names(d)) {
  d$Supporta <- as.numeric(d$Supporta)
  d$Supporta_c <- d$Supporta - mean(d$Supporta, na.rm = TRUE)
}
...
label_single <- function(x) {
  ...
  if (x == "Supporta_c") return("Поддержка власти")
...
f_support <- outcome ~ region + motivation + education + employer + gender + age +
  occupation + language + politics * Supporta_c + appearance
...
coef_support <- coef_map_from_model(models_support$LPM,
                                    c("^politics", "^Supporta_c", ":Supporta_c"))
...
table_moderation(models_support, coef_support, ":Supporta_c",
